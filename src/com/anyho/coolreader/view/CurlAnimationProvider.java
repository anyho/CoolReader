...
class CurlAnimationProvider extends AnimationProvider
{
	...
	protected void drawInternal(Canvas canvas)
	{
		...
		final int x1 = cornerX == 0 ? (dY * dY / dX + dX) / 2 : cornerX
				- (dY * dY / dX + dX) / 2;
		final int y1 = cornerY == 0 ? (dX * dX / dY + dY) / 2 : cornerY
				- (dX * dX / dY + dY) / 2;
		...
		myEdgePaint.setColor(ColorUtils.rgb(ColorUtils
				.getAverageColor(fgBitmap)));
		
		myEdgePath.rewind();
		myEdgePath.moveTo(x, y);
		myEdgePath.lineTo((x + cornerX) / 2, (y + y1) / 2);
		myEdgePath.quadTo((x + 3 * cornerX) / 4, (y + 3 * y1) / 4,
				(x + 7 * cornerX) / 8, (y + 7 * y1 - 2 * sY) / 8);
		myEdgePath.lineTo((x + 7 * x1 - 2 * sX) / 8, (y + 7 * cornerY) / 8);
		myEdgePath.quadTo((x + 3 * x1) / 4, (y + 3 * cornerY) / 4,
				(x + x1) / 2, (y + cornerY) / 2);
		...
		{
			angle = 180 - 180 / 3.1416f * (float) Math.atan2(x - cornerX, y
					- y1);
		}
	...
	PageIndex getPageToScrollTo(int x, int y)
	{
		...
		{
			case leftToRight:
				return myStartX < myWidth / 2 ? PageIndex.next
						: PageIndex.previous;
			case rightToLeft:
				return myStartX < myWidth / 2 ? PageIndex.previous
						: PageIndex.next;
			case up:
				return myStartY < myHeight / 2 ? PageIndex.previous
						: PageIndex.next;
			case down:
				return myStartY < myHeight / 2 ? PageIndex.next
						: PageIndex.previous;
		}
		return PageIndex.current;
	}
